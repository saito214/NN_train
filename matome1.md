...
相互情報量は対称性をもち、I(F; C) = I(C; F)であり、非負である。表２のは相互情報量そのまま  

Mavromatis（2009年、2012年）は、最小記述長の概念を用いたメトリックとして複雑さを音楽コミュニティに導入した。これは、隠れマルコフモデル（HMM）内のクラスタの数を推定するためのものであり、セクション2.6では、HMMと今回の手法を比較する。  
#### 隠れマルコフモデルとは  
* 隠れマルコフモデル（Hidden Markov Model、HMM）は、統計的モデルの一種
* 音声認識、自然言語処理、バイオインフォマティクスなどのさまざまな領域で利用されている
* HMMは、系列データや時系列データのモデリングに使用され隠れ状態と観測状態から構成される
* 各隠れ状態と観測状態は対応している。隠れ状態感の繊維確率を設定することで、次の隠れ状態を予想することができる。  
#### 最小記述法とは
* 最小記述長（Minimum Description Length、MDL）は、情報理論に基づくモデル選択手法の一つ
* データを最も簡単に記述するためのモデルを選択する
* データを記述するために必要なモデルの複雑さと、そのモデルがデータをどれだけ適切に説明できるかという二つの要素を考慮する。
* クラスタリングやモデル選択の問題において広く利用されており、MDLを使用することで、より妥当なモデルを選択し、データの複雑さを考慮した解析が可能になる


### 1.7 精度の定量化
正確さを定量化する方法はいくつかあるが、原則として図１の形式を適用することができる。
ただし、今回は予測と正確さを関連付けている。例えばクラシック音楽の一部がドミナントであれば次にトニックが来ることが予想できる、のような機能情報のみに基づいて音楽刺激について何かを推測する能力を考慮している。
（11）と（12）式なにしてんの？  
現在の表層トークンCnをXと置換し、Y = (C1 , C2 , . . . , Cn−1 , Cn+1 , Cn+2 , . . .)とする。  
もちいた関数F=Fnとすると、FとYの相互情報量はI (F; Y ) = H (Y ) − H (Y |F).となる  
もしFが1対１のマッピングのとき（TSDは７対３）相互情報量は最大になる。つまり、I(F; Y) = I(X; Y)  
(7つの音が７つのパターンに独立しているのなら７つから情報量を計算しても１つから情報量を計算しても変わらない)  
しかし、Y = (C1, C2, ..., Cn-1, Cn+1, Cn+2, ...) は非常に高次元のベクトルであり、相互情報量 I(F; Y) を直接計算することは現実的ではない。よって近似が必要である。一つのアプローチは最近のトークンのみ考慮する方法である。なぜなら隣接するコードが最も重要であるからである。Y→Y'(Y'は限定的)
この仮定の正当性は考慮するべき重要な問題であり、後の2.5で詳しくやる。Ia(F)はすべてのコードに対する平均的な正確さを推定する一つの値である。  
bigramとは
* 任意の文字列が２文字だけ続いた文字列のこと
* この場合は２つのコードの組み合わせ（C Amとか）  
Y' = Yn+1の場合、高次の構造（3つ以上のコードの組み合わせなど）を無視し、現在のコードと次のコードの組み合わせが音楽の予測において最も重要な要素であると仮定することで、その組み合わせの情報量を評価する。高次の構造を無視することにより、計算の複雑さを減らし、相互情報量の評価をより効率的に行うことができるが、高次の構造を無視することによって情報の一部が失われる可能性がある。  
図５(f)では少し異なるアプローチが行われており、カテゴリが他のローカルカテゴリをどれだけよく予測できるかを推定しようとしている。(a)-(d)は次のコードの予測と関連付けられるアプローチであり、(f)のアプローチは次の関数f（次のカテゴリ）を予測するものである。「ドミナントコードはトニックコードに移行する傾向がある」
## 1.8 評価の領域
表２と表３は正確さ（Ia(F)）、複雑さ（Ic(F)）を定義する複数の方法を示しており、それらの組み合わせで評価の領域を形式的に定義することができる。評価の領域は、複雑さと正確さを表すx軸とy軸の二次元グラフであり、定義2の各理論Fは、(x, y) = (Ic(F), Ia(F))で定義される平面上の点に配置することができる。ここで、Ic(F)とIa(F)は、それぞれFに関連付けられた複雑さと正確さの尺度を示す。  
表2と表3からそれぞれ正確さと複雑さのお気に入りの定義を選ぶ。そして、適切なヒストグラムを生成することで、コーパスからp(C)、p(Y), p(F1)、p(F2)、p(F1|C)、p(F2|C)、p(Y|F1)、p(Y|F2)を計算できる。
## 1.9 組み合わせたスコアへの使用
Ia(F)は大きほうが良い。Ic(F)は小さいほうが良い。したがって最良の組み合わせを見つけるためには
L(F)=Ia(F) - λIc(F)が最大になれば良い。この組み合わせスコアL(F)はしばしば複雑さと正確さのトレードオフのラグランジアン（Tishby et al., 1999）と呼ばれます。
## 1.10 最適曲線の計算
ここで、与えられた複雑さに対して最大の正確さを持つ特権的な最適理論のクラスを定義する。  
Fが最適理論である場合、任意の他の理論F'に対して、Ic(F') ≤ Ic(F)であるならば、Ia(F') ≤ Ia(F)が成り立つ。  
最適曲線問題より、複雑さがIc0以下である最適曲線上の理論を見つけることを最適曲線問題と呼ぶ。  
最適曲線を使用して理論を評価することで、最適曲線から遠くに位置する理論（図1のFD、FB、FC）を避け、最適な理論（FA、FE）または近似的に最適な理論（FG、FH）を選択することができ.また、最適な理論自体を独自に調査することもできる。なぜなら、最適な理論は和音の自己発生的なカテゴリ（クラスタ）を提供するからである。過去の研究者は、コーパスデータから機能的なカテゴリを特定するためのさまざまな方法を提案してきました（Quinn＆Mavromatis、2011; Rohrmeier、2005; Rohrmeier＆Cross、2008）。この研究の貢献の一つは、どの表層構造に対してもこの問題を解決できる原理的なフレームワークを提供することである。
##　1.11　最適曲線問題の解決
与えられたコーパスに対して最適な曲線を見つけることは、機械学習におけるよく知られた問題であり、提案された反復アルゴリズムは、数千の表層トークン以下の問題に対して効果的に最適曲線を計算することができる。（付録A参照）
